...
pub async fn list_llm_providers(
    ...
    coordinator
        .run_project_facts_provider(move || {
            ...
            state.with_external_ai_access(&context, |current_context| {
                crate::services::LlmService::list_providers(current_context)?
                    .into_iter()
                    .map(|config| {
                        status_with_secret(current_context, &state.secret_service, config)
                    })
                    .collect()
            })
        })
