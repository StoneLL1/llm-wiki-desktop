...
use crate::models::import_v2::{
    ...
    ImportOperationTaskSummary, ImportRecoveryAction, ImportRecoveryReason, ImportResourceMode,
...
use crate::services::import_v2::execution_control::{BatchOperationState, ImportItemRunOutcome};
...
#[cfg(test)]
use crate::services::import_execution::MAX_IMPORT_WORKER_LIMIT;
use crate::services::import_execution::{
    configured_import_worker_limit, create_temporary_preview_context, current_import_worker_limit,
    enqueue_import_jobs, fail_task_unless_cancelled, finish_batch_worker, preview_authority_error,
    release_temporary_preview_workspace, take_queued_import_jobs, temporary_preview_workspaces,
    BatchOperationJob, ImportWorkerJob, TemporaryPreviewWorkspace, DEFAULT_IMPORT_WORKER_LIMIT,
};
pub(crate) use crate::services::import_execution::{
    import_session_context, is_temporary_preview_session, RESTRICTED_CONTENT_ACK_PATH,
};
        ...
        let worker_result = coordinator
            .run_cancellable(BlockingWorkClass::HeavyIo, cancellation, move || {
                ...
                match result {
                    ...
                    Err(_error) if state.task_service.is_cancelled(&task_id) => {
...
pub(crate) fn dispatch_claimed_import_batch_for_state(
    ...
    let cancellation = state
        .task_service
        .get_cancellation_token(&operation_task_id)
        .unwrap_or_default();
    let failure_app = background_app.clone();
    let failure_task_id = operation_task_id.clone();
    tauri::async_runtime::spawn(async move {
        let worker_result = coordinator
            .run_cancellable(BlockingWorkClass::HeavyIo, cancellation, move || {
    ...
    if previous_status != TaskStatus::WaitingForConfirmation {
        if allow_project_cleanup {
            ...
            if !removed.is_empty() {
                ...
                {
                    enqueue_import_jobs(removed, current_import_worker_limit());
...
pub fn confirm_import_session_v2(
    ...
    let (task, preview_task_ids, canonical_project_identity) = state
        .with_current_project_write_access(
            ...
            |_permit, context| {
                ...
                Ok((task, preview_task_ids, canonical_project_identity))
            },
        )?;
    let task_id = task.id.clone();
    ...
    let failure_task_id = task_id.clone();
    tauri::async_runtime::spawn(async move {
        let git_coordinator = coordinator.clone();
        let git_cancellation = cancellation.clone();
        let worker_result = coordinator
            .run_cancellable(BlockingWorkClass::HeavyIo, cancellation, move || {
                let state = app.state::<AppState>();
                let result = (|| -> Result<TaskResult, BackendError> {
                    ...
                    let batch = state.with_current_project_write_access(
                        ...
                        &request.project_root_path,
                        |permit, _context| {
                            git_coordinator.run_project_git_blocking(
