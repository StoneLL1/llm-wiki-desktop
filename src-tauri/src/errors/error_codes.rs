pub const PATH_TRAVERSAL: &str = "PATH_TRAVERSAL";
pub const PATH_ABSOLUTE_NOT_ALLOWED: &str = "PATH_ABSOLUTE_NOT_ALLOWED";
pub const PATH_OUTSIDE_PROJECT: &str = "PATH_OUTSIDE_PROJECT";
pub const PATH_INVALID: &str = "PATH_INVALID";
pub const PROJECT_CONTEXT_MISMATCH: &str = "PROJECT_CONTEXT_MISMATCH";
pub const IMPORT_V2_SESSION_NOT_FOUND: &str = "IMPORT_V2_SESSION_NOT_FOUND";
pub const IMPORT_V2_SESSION_INVALID: &str = "IMPORT_V2_SESSION_INVALID";
pub const IMPORT_V2_ITEM_NOT_FOUND: &str = "IMPORT_V2_ITEM_NOT_FOUND";
pub const IMPORT_V2_STATE_INVALID: &str = "IMPORT_V2_STATE_INVALID";
pub const IMPORT_V2_SESSION_CURSOR_INVALID: &str = "IMPORT_V2_SESSION_CURSOR_INVALID";
pub const IMPORT_V2_SESSION_CURSOR_STALE: &str = "IMPORT_V2_SESSION_CURSOR_STALE";
pub const IMPORT_V2_SELECTION_STALE: &str = "IMPORT_V2_SELECTION_STALE";
pub const IMPORT_V2_WORK_ITEM_STALE: &str = "IMPORT_V2_WORK_ITEM_STALE";
pub const IMPORT_V2_ENGINE_UNAVAILABLE: &str = "IMPORT_V2_ENGINE_UNAVAILABLE";
pub const IMPORT_V2_ENGINE_PANICKED: &str = "IMPORT_V2_ENGINE_PANICKED";
pub const IMPORT_V2_ENGINE_OUTPUT_INVALID: &str = "IMPORT_V2_ENGINE_OUTPUT_INVALID";
pub const IMPORT_V2_CAPABILITY_INVALID: &str = "IMPORT_V2_CAPABILITY_INVALID";
pub const IMPORT_V2_CAPABILITY_UNAVAILABLE: &str = "IMPORT_V2_CAPABILITY_UNAVAILABLE";
pub const IMPORT_V2_SOURCE_INDEX_INVALID: &str = "IMPORT_V2_SOURCE_INDEX_INVALID";
pub const IMPORT_V2_QUALITY_FAILED: &str = "IMPORT_V2_QUALITY_FAILED";
pub const IMPORT_V2_CANCELLED: &str = "IMPORT_V2_CANCELLED";
pub const IMPORT_V2_COMMIT_CONFLICT: &str = "IMPORT_V2_COMMIT_CONFLICT";
pub const IMPORT_V2_COMMIT_FAILED: &str = "IMPORT_V2_COMMIT_FAILED";
