...
fn package_delete_lists_and_removes_every_owned_artifact_but_keeps_references() {
    ...
    let history = crate::services::VersionHistoryService
        .list(&fixture.context, None, 50)
        .unwrap();
    let operation_id = &history.operations[0].operation_id;
    let record = crate::services::VersionHistoryService
        .load(&fixture.context, operation_id)
        .unwrap();
    ...
    crate::services::VersionHistoryService
        .persist(&fixture.context, &damaged)
        .unwrap();
    assert!(
        ImportV2Service::preview_deleted_source_restore(&fixture.context, operation_id).is_err()
    );
    assert!(!fixture.root.join(&fixture.wiki_path).exists());
    crate::services::VersionHistoryService
        .persist(&fixture.context, &record)
        .unwrap();
    let other = SourcePointer {
    ...
    newer_index
        .by_content_hash
        .insert("new-source-content".into(), other.clone());
    let reserved_hash = saved_index.by_content_hash.keys().next().unwrap();
    newer_index
        .by_content_hash
        .insert(reserved_hash.clone(), other.clone());
    FileStore
        .write_json_atomic(&fixture.context, SOURCE_INDEX_PATH, &newer_index)
        .unwrap();
    assert!(
        ImportV2Service::preview_deleted_source_restore(&fixture.context, operation_id)
            .unwrap()
            .conflicts
            .contains(&SOURCE_INDEX_PATH.to_string())
    );
    ...
    write(
        &fixture.root,
        &fixture.wiki_path,
        b"a newly created file at the same path",
    );
    assert!(
        ImportV2Service::preview_deleted_source_restore(&fixture.context, operation_id)
            .unwrap()
            .conflicts
            .contains(&fixture.wiki_path)
    );
    fs::remove_file(fixture.root.join(&fixture.wiki_path)).unwrap();
    let restore_preview =
        ImportV2Service::preview_deleted_source_restore(&fixture.context, operation_id).unwrap();
    assert!(restore_preview.conflicts.is_empty());
    let expected = restore_preview
        .paths
        .iter()
        .map(|path| {
            (
                path.clone(),
                FileStore
                    .file_hash_if_exists(&fixture.context, path)
                    .unwrap(),
            )
        })
        .collect();
    service
        .restore_deleted_source(&fixture.context, operation_id, &expected)
        .unwrap();
    ...
    assert_eq!(
        restored_index.by_content_hash.get("new-source-content"),
        Some(&other)
    );
    for (key, pointer) in saved_index.by_content_hash {
        assert_eq!(restored_index.by_content_hash.get(&key), Some(&pointer));
    }
...
fn package_delete_rejects_unrestorable_index_bindings_before_deleting_evidence() {
    ...
    index
        .by_content_hash
        .insert("inconsistent-historical-hash".into(), pointer);
    FileStore
        .write_json_atomic(&fixture.context, SOURCE_INDEX_PATH, &index)
        .unwrap();
    assert!(service
        .delete_source(
            &fixture.context,
    ...
    assert!(preview
        .paths
        .iter()
        .all(|entry| fixture.root.join(&entry.path).is_file()));
    assert!(crate::services::VersionHistoryService
        .list(&fixture.context, None, 50)
        .unwrap()
        .operations
        .is_empty());
