...
use crate::models::version_history::{
    SourceDeletionVersion, VersionOperation, VersionOperationKind, VersionOperationState,
...
impl ImportV2Service {
    ...
    fn delete_source_unchecked(
        ...
        let deletion = SourceDeletionVersion {
            ...
            version_id: loaded.version.version_id.clone(),
            wiki_path: loaded.manifest.wiki_path.clone(),
            title: loaded.manifest.title.clone(),
            artifact_kinds: inventory
                .iter()
                .map(|entry| (entry.path.clone(), entry.kind.clone()))
                .collect(),
            by_content_hash: index
                .by_content_hash
                .iter()
                .filter(|(_, pointer)| pointer.source_id == request.source_id)
        ...
        let mut operation = VersionHistoryService.begin(
            context,
            VersionOperationKind::SourceChange,
            &affected_paths,
            None,
        )?;
        ...
        for expected in operation.expected_hashes.values_mut() {
            *expected = None;
        }
        VersionHistoryService.persist(context, &operation)?;
        ...
        let result = (|| {
            ...
            for entry in &inventory {
                transaction.delete_if_hash_matches(
                    &context.resolve_project_path(&entry.path)?,
                    &entry.hash,
                )?;
            }
        ...
        if let Err(failure) = result {
            ...
            if current.iter().all(|(path, bytes)| {
                bytes
                    .as_ref()
                    .map(|bytes| files.content_hash(bytes))
                    .as_ref()
                    == operation.before_hashes.get(path).and_then(Option::as_ref)
            }) && files
                .file_hash(context, &index_path)
                .is_ok_and(|hash| hash == index_hash)
            {
                operation.summary.state = VersionOperationState::Aborted;
                VersionHistoryService.persist(context, &operation)?;
            }
    ...
    pub fn preview_deleted_source_restore(
        context: &ProjectContext,
        id: &str,
    ...
    fn restore_deleted_source_unchecked(
        &self,
        context: &ProjectContext,
        id: &str,
        expected_current: &BTreeMap<String, Option<String>>,
        ...
        if operation.summary.state == VersionOperationState::Restored {
            return Ok(operation);
        }
        let (before, next_index, index_hash, conflicts) =
            prepare_deleted_source_restore(context, &operation)?;
        if !conflicts.is_empty() {
            return Err(deleted_source_conflict(conflicts));
        }
        ...
        let mut current_hashes = current
            .iter()
            .map(|(path, bytes)| {
                (
                    path.clone(),
        ...
        if &current_hashes != expected_current {
            return Err(deleted_source_conflict(
                expected_current.keys().cloned().collect(),
            ));
        }
        let mut restoration = if let Some(restore_id) = operation.restoration_id.as_deref() {
            ...
            let mut restoration = VersionHistoryService.begin(
                context,
                VersionOperationKind::SourceChange,
                &paths,
                None,
            ...
            if restoration
                .before_hashes
                .iter()
                .any(|(path, hash)| current_hashes.get(path) != Some(hash))
            {
                return Err(deleted_source_conflict(paths));
            }
        ...
        transaction.write_if_hash_matches(
            &context.resolve_project_write_path(&index_path)?,
            &pretty_json(&next_index)?,
            &index_hash,
        )?;
        transaction.commit()?;
...
fn deleted_source_conflict(paths: Vec<String>) -> BackendError {
    BackendError::new(
        ...
        "Source paths or index entries are now used by newer content. Current data was preserved.",
        true,
        true,
    )
...
fn validate_deleted_source_index(
    manifest: &SourceManifest,
    deletion: &SourceDeletionVersion,
) -> Result<(), BackendError> {
    let current = current_version(manifest)?;
    if deletion
        .by_content_hash
        .get(&current.content_hash)
        .is_none_or(|pointer| {
            pointer.source_id != manifest.source_id || pointer.version_id != current.version_id
        })
        || deletion.by_content_hash.iter().any(|(hash, pointer)| {
            pointer.source_id != manifest.source_id
                || !manifest.versions.iter().any(|version| {
                    version.version_id == pointer.version_id && &version.content_hash == hash
                })
        })
        || deletion.by_locator.values().any(|pointer| {
            pointer.source_id != manifest.source_id
                || !deletion
                    .by_content_hash
                    .values()
                    .any(|saved| saved == pointer)
        })
    {
        return Err(source_invalid());
    }
...
fn prepare_deleted_source_restore(
    context: &ProjectContext,
    operation: &VersionOperation,
) -> Result<DeletedSourceRestore, BackendError> {
    ...
    if operation.summary.kind != VersionOperationKind::SourceChange
        || deletion
            .artifact_kinds
            .keys()
            .ne(operation.before_hashes.keys())
    {
        return Err(source_invalid());
    }
    ...
    let paths = operation.before_hashes.keys().cloned().collect::<Vec<_>>();
    for path in &paths {
        validate_source_inventory_path(context, path, &deletion.source_id)?;
    }
    ...
    if before.iter().any(|(path, bytes)| {
        bytes
            .as_ref()
            .map(|bytes| FileStore.content_hash(bytes))
            .as_ref()
            != operation.before_hashes.get(path).and_then(Option::as_ref)
    }) {
        return Err(source_invalid());
    ...
    if manifest.source_id != deletion.source_id
        || manifest.current_version_id != deletion.version_id
        || manifest.wiki_path != deletion.wiki_path
    {
        return Err(source_invalid());
    }
    let version = current_version(&manifest)?;
    validate_deleted_source_index(&manifest, deletion)?;
    validate_source_version_binding(
        before
            .get(&manifest.wiki_path)
            .and_then(Option::as_deref)
            .ok_or_else(source_invalid)?,
        &manifest,
        version,
    ...
    .map_err(|_| source_invalid())?;
    let current = VersionHistoryService.capture(context, &paths)?;
    let restoration = operation
        .restoration_id
        .as_deref()
        .map(|id| VersionHistoryService.load(context, id))
        .transpose()?;
    if restoration.as_ref().is_some_and(|record| {
        record.restored_from.as_deref() != Some(operation.summary.operation_id.as_str())
            || record.expected_hashes != operation.before_hashes
    }) {
        return Err(source_invalid());
    }
    ...
    for (path, bytes) in &current {
        ...
        let allowed = if let Some(restoration) = &restoration {
            restoration.before_hashes.get(path) == Some(&hash)
                || restoration.expected_hashes.get(path) == Some(&hash)
        } else {
            hash.is_none()
                || (operation.summary.state == VersionOperationState::Prepared
                    && operation.before_hashes.get(path) == Some(&hash))
        };
        if !allowed {
    ...
    for (saved, current) in [
        (&deletion.by_content_hash, &mut index.by_content_hash),
        (&deletion.by_locator, &mut index.by_locator),
    ] {
        for (key, pointer) in saved {
            if pointer.source_id != deletion.source_id
                || !manifest
                    .versions
                    .iter()
                    .any(|version| version.version_id == pointer.version_id)
            {
                return Err(source_invalid());
            }
            if current.get(key).is_some_and(|current| current != pointer) {
                ...
                current.insert(key.clone(), pointer.clone());
            }
        }
    }
    if conflicts.is_empty() {
