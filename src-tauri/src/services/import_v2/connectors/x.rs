...
pub fn extract_json(
...
) -> Result<XDocument, ConnectorFailure> {
    if !release_enabled {
    ...
    let posts = v["posts"]
        ...
        .iter()
        .filter(|p| {
            p["author"].as_str() == Some(author) && p["relationship"].as_str() != Some("reply")
        })
        .take(12)
        .map(|p| XPost {
            id: p["id"].as_str().unwrap_or("").into(),
            author: author.into(),
            published_at: p["date"].as_str().unwrap_or("").into(),
