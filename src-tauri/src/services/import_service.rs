...
impl ImportService {
    ...
    pub fn preview_import(
        ...
        let known_hashes = self.scan_existing_hashes(context)?;
        ...
        for source_path_str in &request.source_paths {
            ...
            let file_type = Self::classify_file(source_path);

            let archive_dir = Self::target_archive_dir(&file_type);
            let hash = file_hash_fast(source_path).unwrap_or_else(|_| "unknown".to_string());
            ...
            if known_hashes.contains(&hash) {
                ...
                if target_path.exists() {
                    ...
                    if existing_hash != hash {
                        let new_name = Self::deterministic_rename(file_name, &hash);
    ...
    fn scan_existing_hashes(
        ...
        self.collect_hashes(raw_dir, &mut hashes)?;
        Ok(hashes)
    }

    fn collect_hashes(&self, dir: &Path, hashes: &mut HashSet<String>) -> Result<(), BackendError> {
        if !dir.exists() {
            return Ok(());
        }
        ...
        for entry in entries {
            ...
            let path = entry.path();
            if path.is_dir() {
                self.collect_hashes(&path, hashes)?;
...
mod tests {
    ...
    fn classifies_pdf() {
        assert_eq!(
            ImportService::classify_file(Path::new("doc.pdf")),
            SourceFileType::Pdf
        );
        assert_eq!(
            ImportService::classify_file(Path::new("DOC.PDF")),
    ...
    fn classifies_documents() {
        assert_eq!(
            ImportService::classify_file(Path::new("report.docx")),
            SourceFileType::Document
        );
        assert_eq!(
            ImportService::classify_file(Path::new("notes.doc")),
            SourceFileType::Document
        );
        assert_eq!(
            ImportService::classify_file(Path::new("draft.odt")),
            SourceFileType::Document
        );
        assert_eq!(
            ImportService::classify_file(Path::new("legacy.rtf")),
            SourceFileType::Document
        );
    }
    ...
    fn classifies_presentations() {
        assert_eq!(
            ImportService::classify_file(Path::new("deck.pptx")),
            SourceFileType::Presentation
        );
        assert_eq!(
            ImportService::classify_file(Path::new("old.ppt")),
            SourceFileType::Presentation
        );
    }
    ...
    fn classifies_spreadsheets() {
        assert_eq!(
            ImportService::classify_file(Path::new("data.xlsx")),
    ...
    fn classifies_csv_separately() {
        assert_eq!(
            ImportService::classify_file(Path::new("export.csv")),
            SourceFileType::Csv
        );
    }
    ...
    fn classifies_markdown_and_text() {
        assert_eq!(
            ImportService::classify_file(Path::new("notes.md")),
            SourceFileType::Markdown
        );
        assert_eq!(
            ImportService::classify_file(Path::new("readme.txt")),
            SourceFileType::Text
        );
    }
    ...
    fn classifies_images() {
        assert_eq!(
            ImportService::classify_file(Path::new("photo.png")),
            SourceFileType::Image
        );
        assert_eq!(
            ImportService::classify_file(Path::new("logo.svg")),
            SourceFileType::Image
        );
        assert_eq!(
            ImportService::classify_file(Path::new("scan.jpg")),
            SourceFileType::Image
        );
    }
    ...
    fn classifies_html() {
        assert_eq!(
            ImportService::classify_file(Path::new("page.html")),
            SourceFileType::Html
        );
    }
    ...
    fn classifies_unknown() {
        assert_eq!(
            ImportService::classify_file(Path::new("sound.mp3")),
            SourceFileType::Unknown
        );
        assert_eq!(
            ImportService::classify_file(Path::new("archive.zip")),
            SourceFileType::Unknown
        );
    }
    ...
    fn routes_pdf_to_raw_sources_pdfs() {
        assert_eq!(
            ImportService::target_archive_dir(&SourceFileType::Pdf),
            "raw/sources/pdfs"
        );
    }
    ...
    fn routes_docx_to_raw_sources_docs() {
        assert_eq!(
            ImportService::target_archive_dir(&SourceFileType::Document),
            "raw/sources/docs"
        );
    }
    ...
    fn routes_pptx_to_raw_sources_slides() {
        assert_eq!(
            ImportService::target_archive_dir(&SourceFileType::Presentation),
            "raw/sources/slides"
        );
    }
    ...
    fn routes_xlsx_to_raw_sources_sheets() {
        assert_eq!(
            ImportService::target_archive_dir(&SourceFileType::Spreadsheet),
            "raw/sources/sheets"
        );
    }
    ...
    fn routes_md_to_raw_sources_markdown() {
        assert_eq!(
            ImportService::target_archive_dir(&SourceFileType::Markdown),
    ...
    fn routes_images_to_raw_assets() {
        assert_eq!(
            ImportService::target_archive_dir(&SourceFileType::Image),
            "raw/assets"
        );
    }
    ...
    fn routes_unknown_to_raw_sources_other() {
        assert_eq!(
            ImportService::target_archive_dir(&SourceFileType::Unknown),
            "raw/sources/other"
        );
    }
    ...
    fn deterministic_rename_keeps_extension() {
        let renamed = ImportService::deterministic_rename("report.pdf", "abc12345");
    ...
    fn deterministic_rename_handles_no_extension() {
        let renamed = ImportService::deterministic_rename("README", "abc12345");
    ...
    fn deterministic_rename_is_stable() {
        let a = ImportService::deterministic_rename("file.pdf", "abc12345");
        let b = ImportService::deterministic_rename("file.pdf", "abc12345");
        assert_eq!(a, b);
    }
    ...
    fn handles_cjk_filenames_in_classification() {
        assert_eq!(
            ImportService::classify_file(Path::new("概念说明.md")),
            SourceFileType::Markdown
        );
        assert_eq!(
            ImportService::classify_file(Path::new("数据报告.csv")),
            SourceFileType::Csv
        );
        assert_eq!(
            ImportService::classify_file(Path::new("プレゼン.pptx")),
            SourceFileType::Presentation
        );
        assert_eq!(
            ImportService::classify_file(Path::new("研究论文.pdf")),
            SourceFileType::Pdf
        );
    }
    ...
    fn cjk_deterministic_rename_preserves_unicode_stem() {
        let renamed = ImportService::deterministic_rename("概念.md", "abc12345");
