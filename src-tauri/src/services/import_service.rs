...
impl ImportService {
    ...
    fn promote_extracted_to_sources(
        &self,
        context: &ProjectContext,
        file_store: &FileStore,
        preview: &ImportPreview,
...
/// Build a verbatim source page: the imported/extracted text, prefixed with a
/// `type: source` frontmatter block. If the imported text already begins with
/// a YAML frontmatter block, we merge by re-emitting our `type`/`sources`/
/// `title` keys and dropping any conflicting ones from the original.
