...
fn bilibili_asr_grant_is_exact_expiring_and_single_use() {
    ...
    assert!(store
        .take_bilibili_asr("project-a", "session-a", "item-b", exact_url,)
        .unwrap()
        .is_none());
    let mismatch = store
        .take_bilibili_asr(
    ...
    assert!(store
        .has_bilibili_asr("project-a", "session-a", "item-a", exact_url)
        .unwrap());
    assert!(store
        .reserve_bilibili_asr("project-a", "session-a", "item-a", exact_url)
        .unwrap());
    assert!(!store
        .has_bilibili_asr("project-a", "session-a", "item-a", exact_url)
        .unwrap());
    ...
    assert_eq!(
        store
            .take_bilibili_asr("project-a", "session-a", "item-a", exact_url,)
            .unwrap(),
        Some(grant),
    );
    assert!(store
...
fn authenticated_profiles_cannot_cross_project_session_or_item() {
    ...
    store
        .bind_authenticated_profile("project-a", "session-a", "item-a", profile.clone())
        .unwrap();
    assert!(store
        .take_authenticated_profile("project-b", "session-a", "item-a")
        .unwrap()
        .is_none());
    assert!(store
        .take_authenticated_profile("project-a", "session-b", "item-a")
        .unwrap()
        .is_none());
    assert!(store
        .take_authenticated_profile("project-a", "session-a", "item-b")
        .unwrap()
        .is_none());
    assert_eq!(
        store
            .take_authenticated_profile("project-a", "session-a", "item-a")
            .unwrap(),
        Some(profile)
    );
    assert!(store
