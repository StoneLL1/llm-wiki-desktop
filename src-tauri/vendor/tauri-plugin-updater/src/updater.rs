...
impl UpdaterBuilder {
    ...
    /// Sets a hard byte limit for update manifest response bodies.
    pub fn max_manifest_bytes(mut self, max_manifest_bytes: usize) -> Self {
        self.max_manifest_bytes = Some(max_manifest_bytes);
        self
    }
            ...
            match response {
                Ok(res) => {
                    if res.status().is_success() {
                        ...
                        let update_response =
                            read_manifest_response(res, self.max_manifest_bytes).await?;
...
async fn read_manifest_response(
    ...
    if let Some(max_manifest_bytes) = max_manifest_bytes {
        if res
            .content_length()
            .is_some_and(|length| length > max_manifest_bytes as u64)
        {
            return Err(Error::ManifestTooLarge(max_manifest_bytes));
        }
...
mod tests {
    ...
    async fn chunked_manifest_is_rejected_before_unbounded_json_parsing() {
        ...
        let server = tokio::spawn(async move {
            ...
            let body = format!(r#"{{"padding":"{}"}}"#, "x".repeat(128));
            let response = format!(
        ...
        let error = read_manifest_response(response, Some(64))
            .await
            .unwrap_err();
        assert!(matches!(error, Error::ManifestTooLarge(64)));
        server.await.unwrap();
    }
