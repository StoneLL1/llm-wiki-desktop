...
export function WorkflowHistoryView({ runs, onBack, onOpen, onLoadMore }: { runs: WorkflowRun[]; onBack: () => void; onOpen: (taskId: string) => void; onLoadMore: () => void }) {
  ...
  return <div className="workflow-history">
    ...
    <div className="workflow-history__list">{groups.length === 0 ? <p className="workflow-muted">{t("workflows.history.empty")}</p> : groups.map((group) => <section key={group.key}><h3>{t("workflows.history.attempts", { count: group.runs.length })}</h3>{group.runs.map((run) => <button className="workflow-history__run" key={run.taskId} onClick={() => onOpen(run.taskId)} type="button"><span>{t(workflowKindKey(run.kind))}</span><span>{t(workflowStatusKey(run.displayStatus))}</span><time dateTime={run.updatedAt}>{new Date(run.updatedAt).toLocaleString()}</time></button>)}</section>)}</div>
    {historyCursor ? <div className="workflow-actions"><button className="btn btn--secondary" onClick={onLoadMore} type="button">{t("workflows.history.loadMore")}</button></div> : null}
  </div>;
}
