...
export function WorkflowsRightPanel() {
  ...
  return <aside id="right-context-panel" aria-label={t("workflows.context.title")} className="right-panel">
    ...
    <div className="app-pane-scrollbar min-h-0 flex-1 overflow-y-auto px-4 py-3">
      {run ? <>
        <section className="workflow-context-section"><div className="workflow-context-kicker"><AlertTriangle aria-hidden="true" size={13} />{attention ? t("workflows.context.attention") : t("workflows.context.selection")}</div><button className="workflow-context-run" onClick={() => void openRun(run.taskId)} type="button"><strong>{t(workflowKindKey(run.kind))}</strong><span>{t(workflowStatusKey(run.displayStatus))}</span><code>{run.taskId.slice(0, 8)}</code></button></section>
