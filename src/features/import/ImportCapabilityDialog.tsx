    ...
    <div ref={dialogRef} tabIndex={-1} className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 px-4" role="dialog" aria-modal="true" aria-labelledby="import-capability-title">
      <section className="flex max-h-[84vh] w-full max-w-[680px] flex-col rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface-raised)] shadow-lg">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 text-[12px]">
          ...
          {!requirement.installable ? <p className="mt-3 text-[11px] text-[var(--warning-text)]" role="alert">{state.kind === "catalog_unavailable" ? t("importV2.capability.state.catalog_unavailable") : state.kind === "runtime_confinement_unavailable" ? t("importV2.capability.state.runtime_confinement_unavailable") : t("importV2.capability.unavailable")}</p> : null}
          {!["installed", "catalog_unavailable", "runtime_confinement_unavailable", "signed_release_unavailable"].includes(state.kind) ? <label className="mt-3 flex items-start gap-2 rounded-[var(--radius-md)] border border-[var(--border-subtle)] px-3 py-2"><input type="checkbox" aria-label={t("importV2.capability.installAck")} checked={acknowledged} onChange={(event) => setAcknowledged(event.target.checked)} disabled={busy} /><span>{t("importV2.capability.installAck")}</span></label> : null}
          {!["not_installed", "catalog_unavailable", "runtime_confinement_unavailable", "signed_release_unavailable", "installed"].includes(state.kind) ? (
        ...
        <footer className="flex items-center justify-end gap-2 border-t border-[var(--border)] px-4 py-3">
          ...
          {state.kind !== "installed" ? <button type="button" className="btn btn--sm btn--primary" onClick={() => void install()} disabled={!canInstall} title={!requirement.installable ? state.kind === "runtime_confinement_unavailable" ? t("importV2.capability.state.runtime_confinement_unavailable") : t("importV2.capability.unavailable") : undefined}><Download size={13} className="mr-1 inline" aria-hidden="true" />{busy ? <LoaderCircle size={13} className="animate-spin" aria-label={t("importV2.common.loading")} /> : state.kind === "paused" ? t("importV2.capability.resume") : state.kind === "health_check_failed" ? t("importV2.capability.reinstall") : t("importV2.capability.install")}</button> : null}
        </footer>
      </section>
    </div>
  );
