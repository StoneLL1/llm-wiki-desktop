          ...
          <article key={record.entry.id} aria-labelledby={historyDomId("import-history-entry", record.entry.id)} className="border-b border-[var(--border-subtle)] pb-2">
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="m-0 text-[11px] text-[var(--text-muted)]">{t("importV2.history.v2")} · {t("importV2.history.status", { status: t(HISTORY_STATUS_KEYS[record.entry.status] ?? "importV2.history.unknownStatus") })} · {t("importV2.history.itemCount", { count: record.entry.itemCount })}</p>
