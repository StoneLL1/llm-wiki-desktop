  ...
  return (
    <section aria-labelledby="import-history-title" className="border-t border-[var(--border)] px-4 py-3">
      ...
      <div className="mt-2 space-y-2">
        {page.entries.map((entry) => <article key={entry.id} className="border-b border-[var(--border-subtle)] pb-2"><div className="flex items-start gap-2"><FileClock size={14} className="mt-0.5 shrink-0 text-[var(--text-muted)]" aria-hidden="true" /><div className="min-w-0 flex-1"><p className="m-0 truncate text-[12px] font-medium" title={entry.title}>{entry.title}</p><p className="m-0 text-[11px] text-[var(--text-muted)]">{t("importV2.history.v2")} · {t("importV2.history.status", { status: t(HISTORY_STATUS_KEYS[entry.status] ?? "importV2.history.unknownStatus") })}</p><div className="mt-1 flex flex-wrap gap-1.5">{onOpenEntry && entry.availableActions.includes("open_result") ? <button type="button" className="btn btn--sm" onClick={() => onOpenEntry(entry.id)}><FileClock size={12} className="mr-1 inline" aria-hidden="true" />{t("importV2.history.openResult")}</button> : null}{onOpenEntry && entry.availableActions.includes("view_logs") ? <button type="button" className="btn btn--sm" onClick={() => onOpenEntry(entry.id)}><ScrollText size={12} className="mr-1 inline" aria-hidden="true" />{t("importV2.history.viewLogs")}</button> : null}</div></div></div></article>)}
