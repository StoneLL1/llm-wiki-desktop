...
export function ImportMigrationDialog({ open, status, inventory = null, plan, report, confirmation, checkpoint, resumable = false, onScan, onPlan, onApply, onResume, onClose }: ImportMigrationDialogProps) {
  ...
  async function run(key: "scan" | "plan" | "apply" | "resume", action: () => Promise<void> | void) {
    ...
    try {
      ...
      // The workflow owns user-visible error reporting. Consume the rejected
      // action here so a dialog click never creates an unhandled promise.
    } finally {
      setBusy(null);
    }
  }
    ...
    <div ref={dialogRef} tabIndex={-1} className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 px-4" role="dialog" aria-modal="true" aria-labelledby="import-migration-title">
      <section className="flex max-h-[88vh] w-full max-w-[860px] flex-col rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface-raised)] shadow-lg">
        <header className="flex min-h-[52px] items-center gap-3 border-b border-[var(--border)] px-4">
          ...
          <button type="button" className="icon-button" aria-label={t("importV2.migration.close")} title={t("importV2.migration.close")} onClick={onClose}><X size={16} aria-hidden="true" /></button>
        </header>
        <div className="min-h-0 flex-1 overflow-y-auto space-y-4 px-4 py-4 text-[12px]">
          ...
          {report ? <details className="rounded-[var(--radius-md)] border border-[var(--border)] px-3 py-2">
            <summary className="cursor-pointer text-[11px] font-medium">{t("importV2.migration.technicalDetails")}</summary>
            <div className="mt-3 space-y-4">
              <CandidateList title={t("importV2.migration.automaticLinks", { count: report.automaticLinks.length })} candidates={report.automaticLinks} />
              <CandidateList title={t("importV2.migration.proposedRecords", { count: report.proposedRecords.length })} candidates={report.proposedRecords} />
              <CandidateList title={t("importV2.migration.conflicts", { count: report.conflicts.length })} candidates={report.conflicts} />
              <CandidateList title={t("importV2.migration.unmanaged", { count: report.legacyUnmanaged.length })} candidates={report.legacyUnmanaged} />
