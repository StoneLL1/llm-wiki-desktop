...
- Source and release-contract evidence is not packaged-host evidence. Real-machine acceptance follows the owner-approved 2026-09-01 order: the Windows x64 row completed before publication with no blocker, while the macOS arm64, macOS x64, and Ubuntu 24.04 x64 rows remain Pending in the owner-approved tracking issue until actually executed.
