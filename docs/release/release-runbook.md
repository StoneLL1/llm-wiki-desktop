...
5. `packaged-smoke` installs each packaged target and verifies the candidate fixture manifest without contacting the production endpoint. Windows and Linux require the installed process to remain alive through the smoke window. macOS verifies the DMG, installed app bundle metadata, target architecture, executable bit, and Finder-equivalent LaunchServices handoff; it intentionally does not treat the lifetime of a package-internal binary started directly from a headless shell as a user-launch requirement. For `0.2.0`, the owner-approved one-time bootstrap policy replaces only the nonexistent prior-version upgrade with four-platform clean-install acceptance. From `0.2.1`, the real installed production-version-to-candidate update path is mandatory again. The broader recovery, security, and user-journey matrix remains Batch 6 acceptance work; Batch 5 evidence must not be described as that later acceptance.
...
For a late failure after `release-candidate-base` was uploaded, the owner may dispatch the same workflow with an explicit `source_run_id`. This recovery path accepts only a numeric run of this repository's canonical desktop-release workflow whose `head_sha` equals the selected tag commit and whose required candidate, fixture, and four desktop artifacts each still exist exactly once. It skips the already-completed full gate, capability matrix, desktop builds, and manifest assembly; reruns the four parallel packaged-smoke jobs against those immutable artifacts; and then resumes sealing and the same protected atomic publisher. Candidate provenance remains bound to the source run while the new attestation covers the resumed sealed payload.
