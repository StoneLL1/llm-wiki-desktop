...
来源输入采用等宽的“本地文件 / 网页链接”两列，共用无外框布局、输入高度和操作行基线；仅在工作区实际宽度不足时上下排列。“粘贴文本 / Markdown”放在公共操作行。标题和按钮已表达的操作不再重复为分类小字或说明段落；支持格式统一进入能力矩阵。

工作台顶部只保留页签，队列负责进度和筛选数量，固定确认栏负责提交数量。能力矩阵收起时只显示“支持范围”。完成结果和批次采用同一种连续状态行，结果与任务明细默认折叠；待处理、失败、警告和取消 / 重试 / 继续处理入口保持可见。完成摘要位于输入区之后，不遮挡添加入口。

右侧检查器按真实结果显示预览、目标与质量；尚未形成结果时仅显示来源、当前状态和下一步，不堆叠“完成后显示”的占位分区。原始内容说明与技术诊断按需展开。队列精简重复说明时仍保留明确的阻塞原因，响应式排列必须符合虚拟列表固定行高，不能裁切状态和进度。
