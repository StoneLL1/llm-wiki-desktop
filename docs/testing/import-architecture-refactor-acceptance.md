...
| 小红书 `67ea1ed7000000001e00424e` | 实时页面返回结构变化，未生成 Source | 稳定夹具覆盖图文、视频、短链、目标 ID、字幕和 OCR；不替代实时成功 |
...
- 全部格式与平台内容形态逐项使用真实发布 runner 的验收，以及全部平台登录态的实时成功样本，特别是抖音、小红书和 X；不能将稳定夹具通过写作这些平台已经全面可用。
