...
**OpenAI — Codex**| |   
**Cursor — Self-Driving**| |   
**Anthropic — Claude Code**| |  多层记忆系统 + CLAUDE.md 约束，让 Agent 在复杂任务中保持一致性  
...
| |   
---|---|---  
**存储层（在哪）**| |  Layer 0-P 0-T 1 2 3 — 从个人到团队到项目  
**知识类型（是什么）**| |  model decision guideline pitfall process  
**成熟度（多可信）**| |  draft → verified → proven  
...
---|---|---  
**model**| |   
**decision**| |  "选择事件驱动而非 RPC 同步，因为广告状态变更需要解耦"  
...
---|---|---  
**ANALYSE_PRODUCT**| |   
**ANALYSE_TECH**| |  decision, guideline(avoid), pitfall  
**ARCHITECT**| |   
**IMPLEMENT**| |   
**BUILD_VERIFY**| |  pitfall, guideline(avoid)  
...
| | |   
---|---|---|---  
**Layer A: 全景目录**| ` knowledge-catalog.md`| | "知识库有什么？"——分类统计 + 按阶段推荐查阅路径  
**Layer B: 分类清单**| | |  "这个分类有哪些条目？"——每条一行摘要（ID + 标题 + 成熟度 + 标签）  
**Layer C: 完整条目**| ` TK-*.md`| | "这条知识说了什么？"——完整内容 + 背景 + 适用场景  
