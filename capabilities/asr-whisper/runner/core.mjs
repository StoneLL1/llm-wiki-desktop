...
export function isNoAudioExecutionError(error) {
  ...
  return /(?:stream map.*0:a:0.*matches no streams|stream map[^\n]*matches no streams[\s\S]*failed to set value ['"]0:a:0['"]|does not contain any audio stream|no audio stream|audio stream.*not found|failed to find.*audio|failed to load audio)/iu
    .test(details);
}
