...
- `model_base_url`：模型文件的 HTTPS 根目录，可选，默认与程序相同。不同程序版本、平台可以共用同一个模型存储目录。
...
3. 解压归档，使用其中的真实程序执行完整路线与格式 qualification。
4. `split-capability-models.py` 把模型从 ZIP 移出，重新计算程序归档大小和 SHA-256，并写入 `modelFiles`。
