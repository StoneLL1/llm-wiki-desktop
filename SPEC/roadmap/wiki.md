...
- [ ] **PRD-READ-001 文件树支持新建/重命名/删除（隐含）**：现状 = 树只读 + 只能通过编辑器覆盖现有页面（`save_wiki_page` 只处理已存在路径，`save_page_create_new_rejects_existing_file` 测试表明新建路径未被前端使用） → 目标 = 顶栏"+"按钮新建页面（带模板选择），右键行项重命名/删除，删除前创建 Git 检查点并经用户确认 → 涉及文件 = `src/features/wiki/WikiTree.tsx`、`src/features/wiki/wikiStore.ts`、`src-tauri/src/commands/wiki_commands.rs`（需新增 `create_wiki_page` / `rename_wiki_page` / `delete_wiki_page`）、`src-tauri/src/services/search_service.rs` → 验收 = 能新建空页面、重命名会同步更新所有 wikilink 引用、删除走 `ConfirmationDialog` + Git 检查点。
