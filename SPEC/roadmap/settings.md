...
> Workflows 迁移边界：[`../../docs/superpowers/specs/2026-07-30-workflows-panel-redesign.md`](../../docs/superpowers/specs/2026-07-30-workflows-panel-redesign.md) 要求 Agent/BYOK/Provider 配置继续留在 Settings，但用户已确认 Settings 页面首轮保持现状。本文件中的独立 Settings 补全项不得夹带进 Workflows 迁移；迁移只读取既有默认配置并提供缺失配置跳转。
>
> 首次使用与项目打开边界：[`../../docs/superpowers/specs/2026-07-30-first-run-project-open-workbench-design.md`](../../docs/superpowers/specs/2026-07-30-first-run-project-open-workbench-design.md) 已固定启动行为、创建位置和模板语义：有效最近历史自动打开并进入 Dashboard，否则进入无项目工作台；父目录首次为 `Documents/LLM Wiki`、之后记住上次成功位置；模板在创建对话框选择且默认通用，创建后不切换。因此旧设计稿中的“启动行为 / 默认项目位置 / 默认项目模板”不是待补 Settings 项。
...
但完成度约 **55%**，距设计稿差距集中三处：
1. **配置项覆盖严重不足**——通用（外部编辑器/文件关联）、上下文窗口（温度/最大 tokens/滑块）、Agent（超时/安装行为/Skill 自动加载）、后台任务（系统通知/通知点击行为/并发上限）、更新（频率选择/下载安装确认/变更说明）、安全（必需 checkpoint / 人工编辑保护 / Raw Sources 不可变的只读策略状态、兼容库 Git 操作、沙箱说明）、关于——这些在设计稿中是独立 formrow，当前要么缺失、要么字段类型简化。设计稿把若干硬边界画成 toggle 的部分不得照搬。
...
| **通用**（外部编辑器 input / 文件关联 checkbox） | `settings.html:113-178` 仅作行式表单视觉参考；其中启动/位置/模板已由专题规范固定 | “通用”页只显示 projectRoot 路径卡 + scopeCopy 文本卡，无可编辑的外部编辑器或文件关联控件 | ❌缺失 | P1 | `src/features/settings/SettingsView.tsx:167-184` |
...
| **安全**（设计稿为 Git / 人工编辑保护 / Raw Sources / API Key / 沙箱五行） | settings.html:454-515；其中硬边界 toggle 仅作视觉参考 | `SecuritySettings.tsx` 只列出 5 Provider 的密钥状态 + 清除按钮 | ❌缺失 | P0 | 目标改为必需策略只读状态 + 可选兼容库 Git 操作 + 钥匙串/沙箱说明；不得提供绕过硬边界的 toggle |
...
- [ ] **Agent 配置补全（超时/安装行为/Skill）**：设计稿要求任务超时（秒）、安装行为三 checkbox（允许执行安装/仅显示命令复制/检测新 Agent 提示）、Skill 自动加载 toggle + 计数。→ 目标：扩展 `Settings` 类型 + 后端 `SettingsService` 持久化；`AgentSettings.tsx` 加 3 formrow。→ `src/types/settings.ts:9`、`src-tauri/src/models/settings.rs`、`src/features/settings/AgentSettings.tsx`。验收：配置写入 `ProjectLayout.settingsPath`（原生映射为 `.app/settings.json`）并生效，不含密钥。
- [ ] **安全 section 内容对齐硬边界**：当前"安全"页错位为 Provider 密钥列表（应由 LLM Providers 页承担）。→ 目标：把必需 Git checkpoint policy、人工编辑保护与 Raw Sources 默认不可变显示为不可关闭的只读策略状态；仅把“为兼容库初始化本地 Git”等确属用户选择的动作做成明确操作。另展示钥匙串说明 mono 卡与沙箱说明，Provider 密钥清除移到 LLM Providers 页。→ `src/features/settings/SecuritySettings.tsx`。验收：没有 toggle 或 API 能关闭 checkpoint-required 操作、外部编辑保护或原始证据保护。
...
- [ ] **通用 section 补全**：只补外部编辑器与文件关联。→ 目标：扩展 `Settings` + 后端持久化 + formrow。启动、创建父目录和模板遵循专题规范，不新增可冲突的 Settings 开关。→ `src/features/settings/SettingsView.tsx:167-184`、`src-tauri/src/services/settings_service.rs`。验收：外部编辑器与文件关联可编辑并持久化（文件关联属 OS 级，MVP 可只存偏好）。
...
- **`toggle` 使用边界**：Skill 自动加载等真实偏好可以使用 `.toggle`；必需 Git checkpoint、外部编辑保护与 Raw Sources 默认不可变只能显示为只读策略状态，不得因为设计稿使用 toggle 就实现为可关闭配置。
...
1. **P0 硬边界复核（1h）**：确认生产构建不走 `SecretService::memory()`（仅测试用），`AppState` 初始化用默认 `SecretService`（走 keyring）。补集成测试断言所有 layout-defined settings records 均不含 key 明文（原生映射包含 `.app/settings.json`）。→ 已基本达成，补测试即可。
2. **P0 安全 section 对齐（0.5d）**：`SecuritySettings.tsx` 重写为必需 Git checkpoint / 编辑保护 / Raw 不可变的只读策略状态、可选兼容库 Git 操作、钥匙串 mono 说明与沙箱说明；不新增可绕过硬边界的持久化 toggle。Provider 行、掩码、状态徽章与密钥清除已完成，不再列为待办。
