...
1. **P0 · 批量自动修复**：后端 `apply_lint_fixes` + 前端“自动修复 (N)” 按钮。打通 PRD-LINT-003，补齐设计稿顶栏主 CTA。
2. **P0 · error 级别修正**：将死链/index 漂移升级为 `severity: error`，让摘要卡“错误”计数有意义（为下一步铺垫）。
3. **P1 · 摘要卡 + 已通过区**：顶部四宫格 + 列表底部绿色通过条，直接落地 `sumcard` token。
4. **P1 · 模式分段控件**：`mode` store 字段 + seg 控件 + 计数。
5. **P1 · Issue 卡片 tags + 右侧修复按钮**：补齐 severity/source/可自动 badge、target 行、卡片内联“修复”按钮。
6. **P1 · 修复方案 radio + lint-ignore**：扩展 `LintIssue.available_fixes`，新增 `.app/lint-ignore.json`，UI 引入 `check-row` 单选组件。
7. **P1 · 安全检查 checkbox + 设置持久化**：接入 settings_service，默认勾选前两项。
8. **P2 · 详情头部重构 + 行级 diff + Agent 建议卡片**：视觉对齐 `rightpanel__meta`、`diff__line--*`。
9. **P2 · 状态栏 lint 状态 + 键盘 a11y**：footer 计数/上次检查/Git hash；方向键导航与 ARIA。
10. **P2 · 本地规则开关**：`.app/settings.json` 存启用规则集，`run_local_lint` 读取。
