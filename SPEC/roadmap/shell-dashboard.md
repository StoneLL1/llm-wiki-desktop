...
|---|---|---|---|---|---|
| 应用栅格 `.app` | `grid-template-rows: 48px 1fr 28px`，三列 `240px / 1fr / 320px`，支持 `.no-right`、`.right-wide`、`.sidebar-collapsed` 三种变体（`app.css:138-163`） | AppShell 用 `grid-rows-[var(--topbar-h)_1fr_var(--statusbar-h)]` + `grid-cols-[var(--sidebar-w)_minmax(0,1fr)_var(--rightpanel-w)]`；根节点写死 `min-w-[1120px]`；无 collapse / wide 变体切换 | 🟡部分实现 | P1 | src/components/app/AppShell.tsx:127-136 |
...
| 主区头 `.main__header` | 52px、标题 16px + sub 12px mono、右侧 toolbar | 52px、标题 16px、sub 12px mono、primary/secondary 按钮；**缺刷新图标按钮，缺 view.toolbar 分段控件（如 Dashboard 的"刷新/导入资料"）** | 🟡部分实现 | P2 | src/components/app/AppShell.tsx:545-564 |
| 主区 body | padded / padded-lg 两种内边距（`app.css:475-476`） | Dashboard/Import/Lint/Agent/Settings 用 `p-4`；Wiki/Graph/Chat/Exports 用 `overflow-hidden`；**padded-lg（24px）未实现** | 🟡部分实现 | P2 | src/components/app/AppShell.tsx:566 |
...
| 响应式折叠 | 1180px 下右面板变 fixed 抽屉、820px 下侧栏折叠为 56px（`app.css:2069-2121`） | AppShell 写死 `min-w-[1120px]`，**未实现响应式折叠** | ❌缺失 | P2 | src/components/app/AppShell.tsx:127 |
...
- [ ] **响应式折叠**：现状 `min-w-[1120px]` 写死 → 目标（`app.css:2069-2121`） → `src/components/app/AppShell.tsx` + tailwind media query → 验收标准：1180px 下右面板变 fixed 抽屉可折叠；820px 下侧栏折叠为 56px 仅图标。
...
| AppShell `min-w-[1120px]` | 设计稿无此约束，`app.css:146` `min-width: 0` | `AppShell.tsx:127` 写死 `min-w-[1120px]` | ❌ 偏差（破坏窄屏） |
...
8. **[P2] 视觉打磨**：Toast 配色对齐、Drawer 宽度 460、Tooltip 用 `data-tip`、prefers-reduced-motion、Skip link、响应式折叠（去 `min-w-[1120px]`）。
...
- ⚠️ AppShell `min-w-[1120px]` 可能在窄屏违反响应式约束，但 CLAUDE.md 未明确禁止。
