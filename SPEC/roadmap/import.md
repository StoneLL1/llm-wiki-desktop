...
- [ ] **Git 检查点显式开关（P0）**：现状 = 后端 source delete/replace 已走 Git checkpoint；但普通归档确认无 checkpoint 开关 → 目标 = 底部条「创建 Git 检查点」checkbox 默认开启，并在 confirm 前创建 checkpoint → 涉及 `src-tauri/src/commands/import_commands.rs:587-611`、`src-tauri/src/services/import_service/confirmation.rs` → 验收 = 确认导入前有可回滚提交。
...
5. **✅ 已完成 - 底部确认条与编译链路**：`ImportView` 提供「Git 检查点」「导入后触发编译」checkbox；`useImportWorkflow` 串接 `confirm_import_preview` → Wiki scan → 可选 compile，后端 `confirm_import_preview` 接受 `createCheckpoint` 参数。
