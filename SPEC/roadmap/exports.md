...
> Workflows 迁移边界：[`../../docs/superpowers/specs/2026-07-30-workflows-panel-redesign.md`](../../docs/superpowers/specs/2026-07-30-workflows-panel-redesign.md) 规定“生成内容”由统一准备与任务模型启动，但现有 Exports 结果、列表和预览页首轮保持不变。本文件的 Exports UI 打磨项是独立后续工作，不得夹带进 Workflows 迁移；首版也不增加用户自定义模板。
...
**主要落差集中在 UI 层与设计稿的偏差**：
1. 设计稿的“卡片式输出类型选择 + 缩略示意图”被压扁成一个 segmented control；设计稿的 `dlg-export` 信息需求（源页、输出位置、执行路径等）尚未被共享 Generate Content preparation 承接。`dlg-export` 本身已被 Workflows 决策取代，不再作为待实现的独立对话框。
...
| 工具栏：新建导出 | 顶栏 primary 按钮进入共享 Generate Content preparation | 用顶栏内联的 Generate 按钮直接启动 legacy 路径 | 🟡部分实现 | P0（Workflows 侧） | `ExportsView.tsx:182-190` |
...
| 源页选择器 | 共享准备页的类型化适用范围选择器 | 顶栏裸 input（无浏览/校验） | 🟡部分实现 | P0（Workflows 侧） | `ExportsView.tsx:163-171` |
...
| 执行路径 | 在统一 Generate Content 准备页读取 Settings 默认值，允许单次显式覆盖 | Exports 顶栏 route 写死 legacy `auto` | ❌缺失 | P0（Workflows 侧） | `exportStore.ts:27`（`ROUTE_PREFERENCE = "auto"`）、[agent.md](agent.md) |
...
| 统一生成准备 | Workflows Generate Content preparation 承接类型、范围、输出摘要、路线与 Git 策略；不新增 `dlg-export` | 无共享准备入口 | ❌缺失 | P0（Workflows 侧） | `src/features/workflows/` |
...
- [ ] **统一生成准备（Workflows 范围）**：不要在本轮新增 `ExportDialog`。文章页、Exports 与 Workflows 都进入同一个 Generate Content 准备页，由其收集内建类型、适用范围、输出位置摘要和高级执行路径；创建同一种项目任务。
- [ ] **执行路径选择（Workflows 范围）**：移除对 legacy `auto` 的依赖；准备页读取 Settings 默认值，允许本次显式覆盖，所选路径不可用时提示配置且不静默回退。
...
- **键盘可达性**：segmented 按钮和列表项无 `role`/`aria-pressed`。建议：类型卡片用 `role="radio"` + `aria-checked`，进入共享准备页后管理初始焦点与返回焦点；列表项操作按钮已有 `aria-label`（`IconButton`，`ExportsView.tsx:303-316`）继续保持。
...
- **统一准备页 a11y**：Generate Content 使用共享 Workflows preparation；进入后设置语义标题与初始焦点，返回 Exports 时恢复触发点，不新增独立 `ExportDialog`。
...
3. **P0 Generate Content 统一准备页**：在 Workflows feature 中实现，不改造现有 Exports 结果页，也不新增 ExportDialog。
4. **P1 输出类型卡片网格**：按设计稿重做 4 张卡片（SVG 缩略 + 标题 + 描述 + skill folder + 缺模板标记），点击携带类型预设进入共享 Generate Content preparation。
