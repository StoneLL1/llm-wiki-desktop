...
用户有一个包含 PDF、Word、PPT、网页摘录、图片和音视频的资料文件夹。用户选择“打开为项目”，应用初始化项目结构，把能够形成可读来源的内容导入 `raw/` 与 `wiki/sources/`。导入完成后，用户可以查看 Sources，也可以另行选择“用这些来源更新 Wiki”并在完成后从图谱进入知识页面。
...
6. 所有可提交项默认勾选，用户点击“导入到来源库”后写入 `raw/` 和 `wiki/sources/`。
...
| PRD-IMP-005 | 每个成功导入项同时写入原始证据和可读 Source | P0 | 项目中可找到不可变 raw 证据以及对应 `wiki/sources/` 页面 |
