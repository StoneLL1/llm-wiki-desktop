...
用户打开一篇概念页面，生成美化阅读页或知识卡片；或者对整个项目生成 HTML 报告，输出到项目布局解析出的导出目录（原生项目为 `exports/html/`），便于阅读、归档和分享。
...
| PRD-WORKFLOW-005 | 统一跨页面启动 | P0 | Dashboard、Wiki、Lint、Exports 和工作流入口创建同一类项目任务；无项目时不创建任务 |
