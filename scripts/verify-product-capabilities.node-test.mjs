...
test("desktop builds embed the committed resource catalog independently of release provenance", async () => {
  ...
  assert.equal(workflow.includes("LLM_WIKI_CAPABILITY_CATALOG_MODE=distributable"), true);
  assert.equal(workflow.includes("verify-embedded-capability-catalog.mjs"), true);
  assert.equal(workflow.includes("verify-published-capability-assets.mjs"), true);
  assert.equal(buildScript.includes("LLM_WIKI_DISTRIBUTION_TAG"), false);
  assert.equal(workflow.includes("reuse-capability-release.mjs"), false);
});
