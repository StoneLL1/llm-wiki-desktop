...
export async function writeCapabilityContract(options, output, root = repositoryRoot) {
  const product = JSON.parse(await fs.readFile(path.join(root, "capabilities/product-manifest.json"), "utf8"));
  const recipes = JSON.parse(await fs.readFile(path.join(root, "capabilities/release-recipes.json"), "utf8"));
  const sources = JSON.parse(await fs.readFile(path.join(root, "capabilities/release-sources.json"), "utf8"));
  const definition = product.definitions.find((item) => item.capabilityId === options.pack);
  const recipe = recipes.recipes[options.pack];
  if (!definition || !recipe) throw new Error("Capability definition or recipe is missing");
...
export async function stagePreparedCapability(options, root = repositoryRoot) {
  ...
  const contract = await writeCapabilityContract(options, output, root);
